...
public readonly struct Outcome<T> : HK<OutcomeFunctor, T>
{
    ...
    public static implicit operator OutcomeT<Synchronous, T>(Outcome<T> outcome) =>
        outcome.IsSuccess ? Success(outcome.data!) : Failure<T>(outcome.error!);
    ...
    public static implicit operator Outcome<T>(Either<Error, T> value)
        => value.Match(v => new Outcome<T>(v), e => new Outcome<T>(ErrorFrom.Exception(e)));
    ...
    public static Outcome<T> operator |(Outcome<T> ma, OutcomeSideEffect<T> sideEffect) =>
        ma.status == EitherStatus.IsRight ? ma : ma.Map(sideEffect.Run);
